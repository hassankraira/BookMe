...
export class SingupComponent {
  ...
  sendOTP() {
    ...
    if (this.user.email !== '' && this.user.firstName !== '') {
      ...
      this.http.post<{ exists: boolean }>('http://192.168.1.27:3000/check-email', {
        ...
        next: (res) => {
          if (res.exists) {
            ...
            this.http.post('http://192.168.1.27:3000/send-otp', {
              email: this.user.email
            }).subscribe({
              next: () => {
                this.otpSent = true;
  ...
  verifyOTP() {
    this.http.post<any>('http://192.168.1.27:3000/verify-otp', {
      email: this.user.email,
      otp: this.enteredOTP
    }).subscribe({
      next: (res) => {
